document.addEventListener("DOMContentLoaded", function () {
    ...
    const pswConfirmaSenha = document.getElementById("confirmasenha");
    ...
    userInput.addEventListener("input", function () {
        userInput.value = userInput.value.replace(/[^a-z0-9_-]/gi, '');
    });
    ...
    function verificaConfirmaSenha() {
        ...
        if (!senhaInput || !pswConfirmaSenha) {
            return false;
        }

        const valorSenha = senhaInput.value.trim();
        const valorConfirmaSenha = pswConfirmaSenha.value.trim();

        if (valorSenha === valorConfirmaSenha) {
            clearValidation(pswConfirmaSenha);
            return true;
        } else {
            setValidation(pswConfirmaSenha, "Confirmação de senha incorreta");
            return false;
        }
    }
});
